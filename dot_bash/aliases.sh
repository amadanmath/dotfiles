...
then
  if command -v jq >>/dev/null 2>&1 && command -v direnv >>/dev/null 2>&1
  then
    vim() {
      local project_root
      project_root=$(direnv status --json 2>/dev/null \
        | jq -r '.state.foundRC.path // empty' \
        | xargs dirname)

      if [ -n "$project_root" ]; then
        local sockdir="$project_root/.direnv"
        local sockfile="$sockdir/nvim.sock"
        mkdir -p "$sockdir"

        if [ ! -S "$sockfile" ]; then
          nvim --listen "$sockfile" "$@"
          return
        fi
      fi
      nvim "$@"
...
che() {
  ...
  then
    line=$(fc -ln -1)
    mapfile -t args < <(xargs -n1 <<< "$line")
    if [[ "${#args[@]}" -ne 2 || "${args[0]}" != "vim" ]]
    then
      echo 'Please supply `$file` or invoke immediately after `vim $file`' >&2
      return 1
    fi
    file="${args[1]}"
  fi
  chezmoi add "$file"
  chezmoi git add "$(chezmoi source-path "$file")"
  echo Chezmoi: added '`'"$file"'`'.
}

if [[ -x /usr/local/bin/gls ]]
then
  alias ls="/usr/local/bin/gls --color=auto"
elif /bin/ls --color=auto >/dev/null 2>&1
then
